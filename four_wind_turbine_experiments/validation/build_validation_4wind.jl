...
###+
solinfo = DataFrame(CSV.File("./four_wind/RO/training_results_summary.csv"))
...
###+
model = JuMP.read_from_file("/scratch/Users/mkratochvil/fse/four_wind/second_stage_model_scen_1.mps");
##
...
for wb in wind_buses
    winddfdict[wb] = DataFrame(CSV.File("./winddata$(wb).csv"))
end
##
...
###+
cd("/scratch/Users/mkratochvil/validation/RO/v$(variation)_b$(budget)_e$(exp)_n$(nscens)_$(mod)/")
...
for i = 1:4000
    ...
    if isfile(file)
	  ...
	  ###+
        ...
        wind = Dict()
        for wb in wind_buses
            wind[wb] = (1/wmaxdict[wb])*(wrtsdict[wb]/100)*collect(winddfdict[wb][scenarios[i],3:26])
        end
